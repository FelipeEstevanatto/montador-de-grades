      ...
      <div class="grow basis-0 w-full p-3 flex-0100 max-w-full">
        <div class="bg-cardbackground rounded text-white shadow-md">
          ...
          <div class="px-2 py-2 tracking-widerx flex">
            <CustomButton
              class="px-2 h-9 mr-2 border-solid border-1 border-white rounded uppercase font-medium"
              v-if="item.COLOR != '#FF0000'"
            ...
            <CustomButton
              class="px-2 h-9 border-1 border-white rounded uppercase"
              v-if="item.COLOR != '#FF0000'"
              @click="descricao(item)"
              :disabled="loading"
            >
              Descrição
            </CustomButton>
...
export default {
  ...
  data() {
    return {
      ...
      base_address:
        "https://www.unifesp.br/campus/sjc/images/sjc/Secretaria_de_Gradua%C3%A7%C3%A3o/UCs_Vigentes/",
    };
  },
  mounted() {
  ...
  methods: {
    async fetchDisciplinas() {
      try {
        ...
        this.allUnfilteredDisciplines = data.map((item) => ({
          ...item,
          COLOR: "#FF0000",
        }));
      } catch (error) {
    ...
    async atualizarDados() {
      ...
      try {
        ...
        this.allUnfilteredDisciplines.forEach((item_5) => {
          ...
          if (
            !items.some((item_6) => item_6.ID === item_6.ID) &&
            item_5.COLOR !== "#FF0000"
          ) {
            item_5.COLOR = "#FF0000";
